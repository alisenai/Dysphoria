...
def genTrainFiles():
    ...
    for i in range(len(sentences)):
        if i % 2 == 0:
            toFile.write(str(sentences[i]) + "\n")
        else:
            fromFile.write(str(sentences[i]) + "\n")
    # -- Close io and finish --
    fromFile.close()
    toFile.close()
...
if generateModel or generateTrainingFiles:
    print("[Generating training files: " + str(generateTrainingFiles) + "]")
    print("[Generating model: " + str(generateModel) + "]")
